...
def visualize(p, a,b,
  ...
  for person in list_persons:
    ...
    cv2.line(image, keypoints[p1].coordinate, keypoints[p2].coordinate, (0,255,0), 5)
    cv2.line(image, keypoints[p3].coordinate, keypoints[p2].coordinate, (0,255,0), 5)
    ...
    #                 (0, 0, 255), 1)
    cv2.circle(image, keypoints[p1].coordinate, 10, (0,0,255), cv2.FILLED)    
    cv2.circle(image, keypoints[p2].coordinate, 10, (0,0,255), cv2.FILLED)    
    cv2.circle(image, keypoints[p3].coordinate, 10, (0,0,255), cv2.FILLED)
    cv2.circle(image, keypoints[p1].coordinate, 15, (0,0,255), 2)    
    cv2.circle(image, keypoints[p2].coordinate, 15, (0,0,255), 2)    
    cv2.circle(image, keypoints[p3].coordinate, 15, (0,0,255), 2)    

    x1 , y1 = keypoints[p1].coordinate
    x2 , y2 = keypoints[p2].coordinate
    x3 , y3 = keypoints[p3].coordinate
    ...
    before_angle = p
    if (angle - before_angle) > 50:
      angle = before_angle + 10
    if (angle - before_angle) < -50:   
      angle = before_angle - 10


    cv2.putText(image, str(int(angle)), (x2+50 , y2+50), cv2.FONT_HERSHEY_PLAIN, 2, (255,0,255),2)
    per = np.interp(angle, (30,160), (0,100))
    color = (255,255,0)
    bar = np.interp(angle, (40,140), (700,100))
    cv2.rectangle(image, (500, 100), (550, 700), color ,3)
    cv2.rectangle(image, (500, int(bar)), (550, 700), color, cv2.FILLED)
    cv2.putText(image, f'{int(per)}%', (450 ,75), cv2.FONT_HERSHEY_PLAIN, 3, (255,0,0),3)
    count = a
    dir = b
    if per == 100:
      if dir == 0:
    ...
    if per == 0:
      if dir == 1:
        count += 0.5
        dir = 0         
    cv2.putText(image, str(int(count)), (50,150), cv2.FONT_HERSHEY_PLAIN, 7, (0,255,0),10)

  return image, count, dir, angle
